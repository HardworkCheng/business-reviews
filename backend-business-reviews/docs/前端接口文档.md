# 前端接口文档

## 项目：美食点评系统移动端

**版本：** v1.0.0  
**更新时间：** 2024-12-03

---

## 接口规范

### 基础URL

```
开发环境: http://localhost:8080/api
生产环境: https://api.business-reviews.com/api
```

### 请求头

```
Content-Type: application/json
Authorization: Bearer {token}  // 需要登录的接口
```

### 响应格式

**成功响应：**
```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

**错误响应：**
```json
{
  "code": 400,
  "message": "错误信息",
  "data": null
}
```

### 状态码说明

| 状态码 | 说明 |
|-------|------|
| 200 | 成功 |
| 400 | 请求参数错误 |
| 401 | 未登录或token过期 |
| 403 | 无权限 |
| 404 | 资源不存在 |
| 500 | 服务器错误 |

---

## 一、用户认证模块

### 1.1 发送验证码

**接口地址：** `POST /auth/send-code`

**请求参数：**
```json
{
  "phone": "13800138000",
  "type": 1  // 1登录，2注册，3重置密码
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "验证码已发送",
  "data": null
}
```

---

### 1.2 验证码登录

**接口地址：** `POST /auth/login-by-code`

**请求参数：**
```json
{
  "phone": "13800138000",
  "code": "123456"
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "登录成功",
  "data": {
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
    "userInfo": {
      "userId": "1001",
      "username": "美食探店小王",
      "avatar": "https://example.com/avatar.jpg",
      "phone": "13800138000"
    }
  }
}
```

---

### 1.3 第三方登录

**接口地址：** `POST /auth/oauth-login`

**请求参数：**
```json
{
  "type": "wechat",  // wechat/qq/weibo
  "code": "oauth_code"
}
```

**响应示例：** 同1.2

---

### 1.4 退出登录

**接口地址：** `POST /auth/logout`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "退出成功",
  "data": null
}
```

---

## 二、用户信息模块

### 2.1 获取当前用户信息

**接口地址：** `GET /user/info`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "userId": "1001",
    "username": "美食探店小王",
    "avatar": "https://example.com/avatar.jpg",
    "bio": "热爱美食，喜欢探店 🍜",
    "phone": "138****8000",
    "gender": 1,
    "birthday": "1995-01-01",
    "followingCount": 128,
    "followerCount": 3456,
    "likeCount": 12345,
    "favoriteCount": 567,
    "noteCount": 89
  }
}
```

---

### 2.2 更新用户信息

**接口地址：** `PUT /user/info`

**请求头：** 需要 `Authorization`

**请求参数：**
```json
{
  "username": "美食探店小王",
  "avatar": "https://example.com/avatar.jpg",
  "bio": "热爱美食，喜欢探店",
  "gender": 1,
  "birthday": "1995-01-01"
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "更新成功",
  "data": null
}
```

---

### 2.3 获取我的笔记列表

**接口地址：** `GET /user/notes`

**请求头：** 需要 `Authorization`

**查询参数：**
```
pageNum=1&pageSize=10
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "1001",
        "image": "https://example.com/note1.jpg",
        "title": "杭州超好吃的烤肉店！人均100+",
        "likes": 1234,
        "views": 5678,
        "createdAt": "2024-12-01 10:30:00"
      }
    ],
    "total": 89,
    "pageNum": 1,
    "pageSize": 10,
    "hasMore": true
  }
}
```

---

### 2.4 获取我的收藏列表

**接口地址：** `GET /user/favorites`

**请求头：** 需要 `Authorization`

**查询参数：**
```
type=1&pageNum=1&pageSize=10  // type: 1笔记，2商家
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "1001",
        "type": 1,
        "targetId": "2001",
        "image": "https://example.com/note.jpg",
        "title": "笔记标题",
        "createdAt": "2024-12-01 10:30:00"
      }
    ],
    "total": 567,
    "pageNum": 1,
    "pageSize": 10,
    "hasMore": true
  }
}
```

---

### 2.5 获取浏览历史

**接口地址：** `GET /user/history`

**请求头：** 需要 `Authorization`

**查询参数：**
```
pageNum=1&pageSize=20
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "1001",
        "type": 1,
        "targetId": "2001",
        "image": "https://example.com/note.jpg",
        "title": "笔记标题",
        "createdAt": "2024-12-03 09:30:00"
      }
    ],
    "total": 100,
    "hasMore": true
  }
}
```

---

### 2.6 关注用户

**接口地址：** `POST /user/follow`

**请求头：** 需要 `Authorization`

**请求参数：**
```json
{
  "userId": "1002"
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "关注成功",
  "data": null
}
```

---

### 2.7 取消关注

**接口地址：** `DELETE /user/follow/{userId}`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "取消关注成功",
  "data": null
}
```

---

## 三、笔记模块

### 3.1 获取推荐笔记列表

**接口地址：** `GET /notes/recommended`

**查询参数：**
```
pageNum=1&pageSize=10
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "1001",
        "image": "https://example.com/note1.jpg",
        "title": "杭州超好吃的烤肉店！人均100+",
        "author": "美食探店小王",
        "authorAvatar": "https://example.com/avatar.jpg",
        "authorId": "2001",
        "likes": 1234,
        "views": 5678,
        "tag": "热门",
        "tagClass": "tag-hot",
        "createdAt": "2024-12-01 10:30:00"
      }
    ],
    "pageNum": 1,
    "pageSize": 10,
    "hasMore": true
  }
}
```

---

### 3.2 获取笔记详情

**接口地址：** `GET /notes/{id}`

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "id": "1001",
    "image": "https://example.com/note1.jpg",
    "images": [
      "https://example.com/note1-1.jpg",
      "https://example.com/note1-2.jpg"
    ],
    "title": "杭州超好吃的烤肉店！",
    "content": "这家店真的太好吃了，强烈推荐...",
    "author": "美食探店小王",
    "authorAvatar": "https://example.com/avatar.jpg",
    "authorId": "2001",
    "publishTime": "2小时前",
    "tags": ["美食", "探店", "杭州"],
    "likeCount": 1234,
    "commentCount": 128,
    "viewCount": 3456,
    "favoriteCount": 567,
    "isLiked": false,
    "isBookmarked": false,
    "location": "杭州·上塘路",
    "shopId": "3001",
    "shopName": "蔡馬洪涛烤肉",
    "createdAt": "2024-12-01 10:30:00"
  }
}
```

---

### 3.3 发布笔记

**接口地址：** `POST /notes`

**请求头：** 需要 `Authorization`

**请求参数：**
```json
{
  "title": "笔记标题",
  "content": "笔记内容",
  "images": [
    "https://example.com/image1.jpg",
    "https://example.com/image2.jpg"
  ],
  "shopId": "3001",
  "location": "杭州·上塘路",
  "latitude": 30.2741,
  "longitude": 120.1551,
  "tags": ["美食", "探店"],
  "topics": ["1", "2"]
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "发布成功",
  "data": {
    "noteId": "1001"
  }
}
```

---

### 3.4 点赞笔记

**接口地址：** `POST /notes/{id}/like`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "点赞成功",
  "data": {
    "likeCount": 1235
  }
}
```

---

### 3.5 取消点赞笔记

**接口地址：** `DELETE /notes/{id}/like`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "取消点赞成功",
  "data": {
    "likeCount": 1234
  }
}
```

---

### 3.6 收藏笔记

**接口地址：** `POST /notes/{id}/bookmark`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "收藏成功",
  "data": null
}
```

---

### 3.7 取消收藏笔记

**接口地址：** `DELETE /notes/{id}/bookmark`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "取消收藏成功",
  "data": null
}
```

---

### 3.8 删除笔记

**接口地址：** `DELETE /notes/{id}`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "删除成功",
  "data": null
}
```

---

## 四、笔记评论模块

### 4.1 获取笔记评论列表

**接口地址：** `GET /notes/{id}/comments`

**查询参数：**
```
pageNum=1&pageSize=20
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "10001",
        "author": "美食爱好者",
        "authorId": "2002",
        "avatar": "https://example.com/avatar2.jpg",
        "content": "看起来真不错！",
        "time": "2024-12-01 11:00:00",
        "likes": 45,
        "liked": false,
        "replyCount": 3,
        "replies": [
          {
            "id": "10002",
            "author": "美食探店小王",
            "authorId": "2001",
            "avatar": "https://example.com/avatar.jpg",
            "content": "确实很好吃！",
            "time": "2024-12-01 11:05:00",
            "likes": 12,
            "liked": false
          }
        ]
      }
    ],
    "total": 128,
    "hasMore": true
  }
}
```

---

### 4.2 发表评论

**接口地址：** `POST /notes/{id}/comments`

**请求头：** 需要 `Authorization`

**请求参数：**
```json
{
  "content": "评论内容",
  "parentId": null  // 回复评论时填写父评论ID
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "评论成功",
  "data": {
    "commentId": "10001"
  }
}
```

---

### 4.3 点赞评论

**接口地址：** `POST /comments/{id}/like`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "点赞成功",
  "data": {
    "likeCount": 46
  }
}
```

---

### 4.4 取消点赞评论

**接口地址：** `DELETE /comments/{id}/like`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "取消点赞成功",
  "data": {
    "likeCount": 45
  }
}
```

---

### 4.5 删除评论

**接口地址：** `DELETE /comments/{id}`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "删除成功",
  "data": null
}
```

---

## 五、商家模块

### 5.1 获取商家列表

**接口地址：** `GET /shops`

**查询参数：**
```
category=美食
&sortField=distance  // distance/popularity/rating/price
&sortOrder=asc  // asc/desc
&keyword=烤肉
&latitude=30.2741
&longitude=120.1551
&pageNum=1
&pageSize=10
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "3001",
        "name": "蔡馬洪涛烤肉·老北京铜锅涮羊肉",
        "image": "https://example.com/shop1.jpg",
        "rating": 4.8,
        "reviewCount": 1234,
        "tags": ["烤肉", "火锅", "人气榜"],
        "location": "拱墅区",
        "address": "上塘路1035号",
        "distance": "1.2km",
        "popularity": 9876,
        "averagePrice": 120.00,
        "latitude": 30.2751,
        "longitude": 120.1561
      }
    ],
    "total": 150,
    "hasMore": true
  }
}
```

---

### 5.2 获取商家详情

**接口地址：** `GET /shops/{id}`

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "id": "3001",
    "name": "蔡馬洪涛烤肉·老北京铜锅涮羊肉",
    "headerImage": "https://example.com/shop1-header.jpg",
    "images": [
      "https://example.com/shop1-1.jpg",
      "https://example.com/shop1-2.jpg"
    ],
    "description": "正宗老北京烤肉，精选优质食材...",
    "rating": 4.6,
    "reviewCount": 1460,
    "tasteScore": 4.9,
    "environmentScore": 4.8,
    "serviceScore": 4.7,
    "address": "上塘路1035号（中国工商银行旁）",
    "businessHours": "11:30 - 03:00",
    "phone": "0571-12345678",
    "averagePrice": 120.00,
    "tags": ["烤肉", "火锅", "人气榜", "停车方便"],
    "isFavorited": false,
    "latitude": 30.2751,
    "longitude": 120.1561,
    "distance": "1.2km"
  }
}
```

---

### 5.3 获取商家评价列表

**接口地址：** `GET /shops/{id}/reviews`

**查询参数：**
```
pageNum=1&pageSize=10&sortBy=latest  // latest/rating
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "20001",
        "author": "美食爱好者",
        "authorId": "2002",
        "avatar": "https://example.com/avatar2.jpg",
        "rating": 5.0,
        "tasteScore": 5.0,
        "environmentScore": 4.5,
        "serviceScore": 5.0,
        "date": "2024-11-30",
        "content": "味道非常棒，环境也很好！",
        "images": [
          "https://example.com/review1-1.jpg",
          "https://example.com/review1-2.jpg"
        ],
        "likeCount": 23
      }
    ],
    "total": 1460,
    "hasMore": true
  }
}
```

---

### 5.4 发表商家评价

**接口地址：** `POST /shops/{id}/reviews`

**请求头：** 需要 `Authorization`

**请求参数：**
```json
{
  "rating": 5.0,
  "tasteScore": 5.0,
  "environmentScore": 4.5,
  "serviceScore": 5.0,
  "content": "味道非常棒！",
  "images": [
    "https://example.com/review1.jpg",
    "https://example.com/review2.jpg"
  ]
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "评价成功",
  "data": {
    "reviewId": "20001"
  }
}
```

---

### 5.5 收藏商家

**接口地址：** `POST /shops/{id}/favorite`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "收藏成功",
  "data": null
}
```

---

### 5.6 取消收藏商家

**接口地址：** `DELETE /shops/{id}/favorite`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "取消收藏成功",
  "data": null
}
```

---

## 六、分类模块

### 6.1 获取分类列表

**接口地址：** `GET /categories`

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": [
    {
      "id": 1,
      "name": "美食",
      "icon": "🍜",
      "color": "#FFD166"
    },
    {
      "id": 2,
      "name": "KTV",
      "icon": "🎤",
      "color": "#EF476F"
    }
  ]
}
```

---

## 七、消息模块

### 7.1 获取聊天列表

**接口地址：** `GET /messages/chats`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": [
    {
      "id": "5001",
      "name": "小红",
      "userId": "2002",
      "avatar": "https://example.com/avatar2.jpg",
      "lastMessage": "这家店真的超级好吃！",
      "time": "10:30",
      "unread": 2,
      "online": true
    }
  ]
}
```

---

### 7.2 获取聊天消息

**接口地址：** `GET /messages/chats/{sessionId}`

**请求头：** 需要 `Authorization`

**查询参数：**
```
pageNum=1&pageSize=20
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "60001",
        "fromUserId": "2002",
        "toUserId": "2001",
        "content": "这家店真的超级好吃！",
        "messageType": 1,
        "isRead": true,
        "createdAt": "2024-12-03 10:30:00"
      }
    ],
    "hasMore": false
  }
}
```

---

### 7.3 发送消息

**接口地址：** `POST /messages/chats/{userId}`

**请求头：** 需要 `Authorization`

**请求参数：**
```json
{
  "content": "你好！",
  "messageType": 1  // 1文本，2图片，3语音
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "发送成功",
  "data": {
    "messageId": "60001"
  }
}
```

---

### 7.4 获取系统通知列表

**接口地址：** `GET /messages/notices`

**请求头：** 需要 `Authorization`

**查询参数：**
```
pageNum=1&pageSize=20
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "70001",
        "user": "美食达人",
        "userId": "2003",
        "avatar": "https://example.com/avatar3.jpg",
        "action": "点赞了你的笔记",
        "time": "5分钟前",
        "icon": "❤️",
        "type": "like",
        "image": "https://example.com/note.jpg",
        "targetId": "1001",
        "isRead": false
      },
      {
        "id": "70002",
        "user": "探店小能手",
        "userId": "2004",
        "avatar": "https://example.com/avatar4.jpg",
        "action": "评论了你的笔记",
        "time": "10分钟前",
        "icon": "💬",
        "type": "comment",
        "image": "https://example.com/note2.jpg",
        "targetId": "1002",
        "isRead": false
      },
      {
        "id": "70003",
        "user": "美食分享家",
        "userId": "2005",
        "avatar": "https://example.com/avatar5.jpg",
        "action": "关注了你",
        "time": "1小时前",
        "icon": "👤",
        "type": "follow",
        "image": null,
        "targetId": null,
        "isRead": true
      }
    ],
    "total": 50,
    "unreadCount": 12,
    "hasMore": true
  }
}
```

---

### 7.5 标记通知为已读

**接口地址：** `PUT /messages/notices/read`

**请求头：** 需要 `Authorization`

**请求参数：**
```json
{
  "noticeIds": ["70001", "70002"]  // 为空数组表示全部已读
}
```

**响应示例：**
```json
{
  "code": 200,
  "message": "标记成功",
  "data": null
}
```

---

### 7.6 获取未读消息数

**接口地址：** `GET /messages/unread-count`

**请求头：** 需要 `Authorization`

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "chatUnread": 5,
    "noticeUnread": 12,
    "totalUnread": 17
  }
}
```

---

## 八、上传模块

### 8.1 上传图片

**接口地址：** `POST /upload/image`

**请求头：** 
```
Content-Type: multipart/form-data
Authorization: Bearer {token}
```

**请求参数：**
```
file: 图片文件（支持JPG、PNG格式）
```

**响应示例：**
```json
{
  "code": 200,
  "message": "上传成功",
  "data": {
    "url": "https://example.com/uploads/image123.jpg"
  }
}
```

---

### 8.2 批量上传图片

**接口地址：** `POST /upload/images`

**请求头：** 
```
Content-Type: multipart/form-data
Authorization: Bearer {token}
```

**请求参数：**
```
files: 图片文件数组（最多9张）
```

**响应示例：**
```json
{
  "code": 200,
  "message": "上传成功",
  "data": {
    "urls": [
      "https://example.com/uploads/image123.jpg",
      "https://example.com/uploads/image124.jpg"
    ]
  }
}
```

---

## 九、话题模块

### 9.1 获取热门话题

**接口地址：** `GET /topics/hot`

**查询参数：**
```
pageNum=1&pageSize=10
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "id": "8001",
        "name": "杭州美食探店",
        "description": "分享杭州地区的美食探店体验",
        "coverImage": "https://example.com/topic1.jpg",
        "noteCount": 12345,
        "viewCount": 567890,
        "isHot": true
      }
    ],
    "hasMore": true
  }
}
```

---

### 9.2 搜索话题

**接口地址：** `GET /topics/search`

**查询参数：**
```
keyword=美食&pageNum=1&pageSize=10
```

**响应示例：** 同9.1

---

## 十、地图模块

### 10.1 获取附近商家

**接口地址：** `GET /map/nearby-shops`

**查询参数：**
```
latitude=30.2741
&longitude=120.1551
&category=美食
&radius=5000  // 搜索半径（米）
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": [
    {
      "id": "3001",
      "name": "蔡馬洪涛烤肉",
      "rating": 4.6,
      "reviewCount": 1460,
      "distance": "170m",
      "image": "https://example.com/shop1.jpg",
      "latitude": 30.2751,
      "longitude": 120.1561,
      "tags": ["烤肉", "火锅"]
    }
  ]
}
```

---

## 十一、搜索模块

### 11.1 综合搜索

**接口地址：** `GET /search`

**查询参数：**
```
keyword=烤肉
&type=all  // all/note/shop
&pageNum=1
&pageSize=10
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "notes": {
      "list": [],
      "total": 50
    },
    "shops": {
      "list": [],
      "total": 30
    }
  }
}
```

---

### 11.2 搜索建议

**接口地址：** `GET /search/suggest`

**查询参数：**
```
keyword=烤
```

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "suggestions": [
      "烤肉",
      "烤鱼",
      "烤串"
    ]
  }
}
```

---

### 11.3 热门搜索

**接口地址：** `GET /search/hot`

**响应示例：**
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "hotWords": [
      "火锅",
      "烧烤",
      "日料",
      "西餐"
    ]
  }
}
```

---

## 错误码说明

| 错误码 | 说明 |
|-------|------|
| 40001 | 手机号格式错误 |
| 40002 | 验证码错误或已过期 |
| 40003 | 用户不存在 |
| 40004 | 参数缺失 |
| 40101 | 未登录 |
| 40102 | Token已过期 |
| 40103 | Token无效 |
| 40301 | 无权限操作 |
| 40401 | 资源不存在 |
| 40402 | 笔记不存在 |
| 40403 | 商家不存在 |
| 50001 | 服务器内部错误 |
| 50002 | 数据库错误 |

---

## 附录

### 时间格式

- 所有时间字段统一使用ISO 8601格式：`YYYY-MM-DD HH:mm:ss`
- 相对时间展示：刚刚、5分钟前、1小时前、昨天、2024-12-01

### 图片规格

- 头像：200x200 ~ 400x400
- 笔记封面：800x1000（4:5比例）
- 商家封面：1200x800（3:2比例）
- 图片格式：JPG、PNG
- 单张图片大小：不超过5MB

### 分页说明

- 默认每页10条数据
- 最大每页50条数据
- 返回hasMore字段表示是否还有更多数据

---

**文档版本：** v1.0.0  
**最后更新：** 2024-12-03
