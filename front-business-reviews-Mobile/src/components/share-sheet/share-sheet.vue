<template>
	<view class="share-wrapper" :class="{ 'visible': visible }">
		<!-- 半透明遮罩 -->
		<view 
			class="overlay-mask" 
			:class="{ 'show': visible }"
		...
		>
			...
			<view class="note-preview">
				<image 
					v-if="noteInfo.coverImage"
					:src="noteInfo.coverImage" 
					class="note-cover" 
					mode="aspectFill"
				></image>
				<view class="note-info">
					<text class="note-title">{{ noteInfo.title || '无标题' }}</text>
					<text class="note-content">{{ noteInfo.content || '暂无内容' }}</text>
				</view>
			</view>
...
const emit = defineEmits(['update:visible', 'close', 'share-success'])
...
watch(() => props.visible, (newVal) => {
	if (newVal && !hasLoadedUsers.value) {
		fetchUserLists()
	}
})
